...
import { loadConcertData } from '../../hooks/usePortfolioSummary'
  ...
  useEffect(() => {
    ...
    loadConcertData()
      ...
      .catch((loadError: unknown) => {
        if (!isActive) return
        setError(
          loadError instanceof Error ? loadError : new Error('The concert archive failed to load'),
        )
      })
